...
DateUtil.prototype.analyzeTime = function (timestamp) {
  ...
  if (syear - timeyear != 0 && stoday.getTime() - timestamp > sevenday) {
    //跨年大于七天
    timeStr =
      ...
      '日'
    return timeStr
  } else if (stoday.getTime() - timestamp > sevenday) {
    //大于七天不跨年
    timeStr = timeData.getMonth() + 1 + '月' + timeData.getDate() + '日'
  } else if (
    stoday.getTime() - timestamp < sevenday &&
    stoday.getTime() - timestamp > oneday
  ) {
    //一周内不是昨天
    timeStr =
      '星期' +
      ...
      (timeData.getHours() > 9 ?
        ...
        '0' + timeData.getHours()) +
      ':' +
      (timeData.getMinutes() > 9 ?
        timeData.getMinutes() :
        '0' + timeData.getMinutes())
  } else if (
    stoday.getTime() - timestamp < oneday &&
    stoday.getTime() - timestamp > 0
  ) {
    //昨天
    timeStr =
      '昨天 ' +
...
/**
 * 返回指定范围的日期列表
 * @param begin, end (Date类型)
 * @returns {Array}
 */
DateUtil.prototype.getDates = function (begin, end) {
  let dates = []
  var unixDb = begin.getTime()
  var unixDe = end.getTime()
  for (var k = unixDb; k <= unixDe;) {
    dates.push(this.dateToStr('yyyy-MM-dd', new Date(parseInt(k))))
    k = k + 24 * 60 * 60 * 1000
  }
  return dates
}
 ...
 * @returns {Array}
 */
DateUtil.prototype.getMonths = function (begin, end) {
  let yearBegin = begin.getFullYear()
  let monthBegin = begin.getMonth() + 1
  let yearEnd = end.getFullYear()
  let monthEnd = end.getMonth() + 1
  let dates = []
  for (let y = yearBegin; y <= yearEnd; y++) {
    for (let m = 1; m <= 12; m++) {
      if (y === yearBegin && m < monthBegin) {
        continue
      }
      if (y === yearEnd && m > monthEnd) {
        continue
      }
      let mm = m.toString()
      if (m < 10) {
      ...
      dates.push(y.toString() + '-' + mm)
    }
  }
  return dates
}

//改变日期格式 2018-11-27T03:00:29.443+0000  ==>  2018/11/27 11:00:29
DateUtil.prototype.changeDateFormat = function (dateStr) {
  let createdTime = ''
  if (dateStr) {
    ...
    dateStr = dateStr.splice(26, 0, ':')
    let dateee = new Date(dateStr).toJSON()
    createdTime = new Date(+new Date(dateee) + 8 * 3600 * 1000)
      .toISOString()
      .replace(/-/g, '-')
      .replace(/T/g, ' ')
      .replace(/\.[\d]{3}Z/, '')
